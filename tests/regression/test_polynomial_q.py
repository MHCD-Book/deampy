...
for i in range(N):
    q_cont.update(y=fs[i], values_of_continuous_features=X[i])
...
for i in range(N):
    q_cat.update(y=hs[i], values_of_indicator_features=I[i])
...
for i in range(N):
    q_cont_cat.update(y=gs[i], values_of_continuous_features=X[i], values_of_indicator_features=I[i])
